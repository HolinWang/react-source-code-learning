 ...
 *
 * 注意：此实现较为简化，不处理 key/ref、函数组件（后续可扩展支持）和事件处理器特殊绑定。
 */
...
function performUnitOfWork(fiber) {
  ...
  // 确保 fiber.props 存在并统一为可安全读取的对象
  fiber.props = fiber.props || { children: [] };
  const props = fiber.props || {};
  // 将非 children 的 props 赋值到 dom（updateProperties 由 domUtils.js 实现）
  updateProperties(fiber.dom, props);
  ...
  const flatChildren = normalizeChildren(props.children);
